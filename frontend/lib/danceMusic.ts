...
/**
 * Speaks "Anara siap!" using browser SpeechSynthesis with a cheerful female voice.
 */
export function speakAnaraReady(onStart?: () => void, onEnd?: () => void, phrase: string = "Anara siap!") {
  ...
  utterance.rate = 1.12; // slightly fast & energetic
  utterance.pitch = 1.25; // cheerful female pitch

  // Pick an Indonesian female voice if available in user's OS/browser
  const voices = window.speechSynthesis.getVoices();
  const idVoice = voices.find((v) => v.lang.includes("id") || v.name.toLowerCase().includes("indonesia") || v.name.toLowerCase().includes("gadis") || v.name.toLowerCase().includes("female"));
  if (idVoice) {
