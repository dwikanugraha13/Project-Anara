...
- Give it a name like "anara-agent"
- Select scopes:
...
- Give it a title like "anara-agent-<machine-name>"
...
elif _hermes_env="${ANARA_HOME:-$HOME/.anara}/.env"; [ -f "$_hermes_env" ] && grep -q "^GITHUB_TOKEN=" "$_hermes_env"; then
