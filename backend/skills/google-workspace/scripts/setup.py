#!/usr/bin/env python3
"""Google Workspace OAuth2 setup for Anara Agent.
...
ANARA_HOME = get_hermes_home()
TOKEN_PATH = ANARA_HOME / "google_token.json"
CLIENT_SECRET_PATH = ANARA_HOME / "google_client_secret.json"
PENDING_AUTH_PATH = ANARA_HOME / "google_oauth_pending.json"
...
def _format_missing_scopes(missing_scopes: list[str]) -> str:
    ...
    return (
        ...
        f"{bullets}\n"
        "Run the Google Workspace setup again from this same Anara profile to refresh consent."
    )
...
def install_deps():
    ...
    # Fallback: the interpreter has no pip (the Anara Docker image's venv is
    ...
    print(
        "On environments without pip (e.g. Nix, or the Anara Docker image's "
...
def store_client_secret(path: str):
    """Copy and validate client_secret.json to Anara home."""
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace OAuth setup for Anara")
