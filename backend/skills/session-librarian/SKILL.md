...
| List/filter by metadata (age, source, cost, tokens, workspace) | `anara sessions list` / `stats` via terminal |
| Rename | `anara sessions rename <session_id> <title...>` |
| Bulk soft-hide (reversible) | `anara sessions archive <filters>` |
| Delete (destructive) | `anara sessions delete` / `anara sessions prune <filters>` |
| Export before deleting anything valuable | `anara sessions export --session-id <id> --format md` |
...
`anara sessions list --source telegram --limit 50` instead.
...
- Before deleting anything with meaningful content, offer
  `anara sessions export --format md` as a backup.
...
After a cleanup pass, re-run the discovery query and `anara sessions list`
