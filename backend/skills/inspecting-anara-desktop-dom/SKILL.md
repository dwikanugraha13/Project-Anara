---
name: inspecting-anara-desktop-dom
description: "Read the live Anara desktop DOM/CSS over CDP."
version: 1.0.0
author: Anara Agent
license: MIT
...
# Inspecting the live Anara desktop DOM
...
`debugging-anara-desktop`), or anything where the real question is "does this
look right".
...
- **A throwaway `HERMES_HOME` has no backend.** The app logs `ECONNREFUSED` for
  `anara:api` and may exit on its own. The renderer still mounts and the DOM is
