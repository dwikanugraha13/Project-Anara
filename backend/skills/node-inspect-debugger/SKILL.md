...
6. **Running `node inspect` through an agent terminal.** It's a PTY-friendly REPL. In Anara, launch it with `terminal(pty=true)` or `background=true` + `process(action='submit', data='...')`. Non-PTY foreground mode will work for one-shot commands but not for interactive stepping.
