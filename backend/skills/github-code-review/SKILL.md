...
curl -s -X POST \
  ...
  -d "{
    \"commit_id\": \"$HEAD_SHA\",
    \"event\": \"COMMENT\",
    \"body\": \"Code review from Anara Agent\",
    \"comments\": [
...
```bash
source "${HERMES_HOME:-$HOME/.anara}/skills/github/github-auth/scripts/gh-env.sh"
...
gh pr review $PR_NUMBER --approve --body "Reviewed by Anara Agent. Code looks clean — good test coverage, no security concerns."
